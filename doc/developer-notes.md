...
- [Development guidelines](#development-guidelines)
    - [General Blessing Core](#general-blessing-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Blessing Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play BLESs (tBLES)" on the test network, if you
...
Blessing Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/blessing-project/blessing/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_blessing
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_blessing --log_level=test_suite
$ valgrind -v --leak-check=full src/blessingd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_blessing.coverage/index.html`.
```
...
- BitcoinMiner : Generates BLESs (if wallet is enabled).
...
However, in open source software such as Blessing Core, where everyone uses
...
pay attention to for reviewers of Blessing Core code.

General Blessing Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Blessing Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Blessing Core so that they can be integrated
...
file descriptors it will cause problems with Blessing's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:Blessing-Project/Blessing.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `blessing-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `blessing-cli` and the GUI debug console use this table to determine how to
