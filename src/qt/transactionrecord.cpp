...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZBLESModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zblesTracker->HasMintTx(hash))) {
            //zBLES stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZBLES;
...
bool IsZBLESType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZBLES:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZBLES || type == TransactionRecord::MNReward) {
