...
#include "ui_zblescontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zblesTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zblesTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zBLES.";
            else if (!mint.isSeedCorrect)
                strReason = "The zBLES seed used to mint this zBLES is not the same as currently hold in the wallet";
            else
