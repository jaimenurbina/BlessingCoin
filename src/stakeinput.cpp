...
// The zBLES block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zBLES is a hash of the serial
...
{
    //Create an output returning the zBLES that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZBLESOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zBLES output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zBLES", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZBLESOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zBLES output", __func__);
...
{
    CzBLESTracker* zblesTracker = pwallet->zblesTracker.get();
    CMintMeta meta;
    if (!zblesTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zblesTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a BLES stake is the outpoint
