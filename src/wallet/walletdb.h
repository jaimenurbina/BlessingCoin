...
public:
    ...
    bool WriteZBLESSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBLESSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBLESSeed_deprecated(uint256& seed);
    bool EraseZBLESSeed();
    bool EraseZBLESSeed_deprecated();

    bool WriteZBLESCount(const uint32_t& nCount);
    bool ReadZBLESCount(uint32_t& nCount);
