...
CzBLESTracker::CzBLESTracker(std::string strWalletFile)
{
...
CzBLESTracker::~CzBLESTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBLESTracker::Init()
{
...
bool CzBLESTracker::Archive(CMintMeta& meta)
{
...
bool CzBLESTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBLESTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBLESTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBLESTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzBLESTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzBLESTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzBLESTracker::GetSerialHashes()
{
...
CAmount CzBLESTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBLESTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBLESTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBLESTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBLESTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBLESTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBLESTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBLESTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBLESTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBLESTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBLESTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzBLESWallet* zBLESWallet)
{
    bool iszBLESWalletInitialized = (NULL != zBLESWallet);
    CMintMeta meta;
    ...
    if (! iszBLESWalletInitialized)
        zBLESWallet = new CzBLESWallet(strWalletFile);
    meta.isSeedCorrect = zBLESWallet->CheckSeed(dMint);
    if (! iszBLESWalletInitialized)
        delete zBLESWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzBLESTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBLESTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBLESTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBLESTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBLESTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBLESTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzBLESWallet* zBLESWallet = new CzBLESWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zBLESWallet);
        }
        delete zBLESWallet;
        LogPrint("zero", "%s: added %d dzbles from DB\n", __func__, listDeterministicDB.size());
    }
