...
CzBLESWallet::CzBLESWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbles seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBLESSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBLESSeed_deprecated()) {
                    LogPrintf("%s: Updated zBLES seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbles seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbles wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBLESWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBLESCount(nCountLastUsed);
    else if (!walletdb.ReadZBLESCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBLESWallet::Lock()
{
    seedMaster = 0;
}

void CzBLESWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBLESWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBLES(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBLESWallet::LoadMintPoolFromDB()
{
...
void CzBLESWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBLESWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBLESWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zblesTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBLESWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBLES(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zblesTracker which also adds to database
    pwalletMain->zblesTracker->Add(dMint, true);
...
void CzBLESWallet::SeedToZBLES(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBLESWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBLESWallet::GenerateDeterministicZBLES(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBLESWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBLESWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBLESWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
